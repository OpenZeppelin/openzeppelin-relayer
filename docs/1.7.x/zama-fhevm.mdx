...
For signer configuration details, see the [Signers](/relayer/1.7.x/configuration/signers) guide.
...
For a full working walkthrough, see the [Zama FHEVM Counter Guide](/relayer/1.7.x/guides/zama-fhevm-counter-guide).
...
| Method | Purpose |
| --- | --- |
| [Send Transaction](/relayer/1.7.x/api/sendTransaction) | Submit an encrypted FHEVM contract call on-chain |
| [Sign Typed Data](/relayer/1.7.x/api/signTypedData) | Sign the EIP-712 payload used for user decryption |
| [Get Relayer](/relayer/1.7.x/api/getRelayer) | Retrieve the OpenZeppelin Relayer's EVM address (needed by the Zama SDK for input proofs and to seed the signer's wallet account) |
| [Get Transaction by ID](/relayer/1.7.x/api/getTransactionById) | Poll for transaction status after submission |
...
The example contract is deployed from [Zama's fhevm-hardhat-template](https://github.com/zama-ai/fhevm-hardhat-template). For a full walkthrough, see the [Zama FHEVM Counter Guide](/relayer/1.7.x/guides/zama-fhevm-counter-guide).
...
- [Zama FHEVM Counter Guide](/relayer/1.7.x/guides/zama-fhevm-counter-guide)
