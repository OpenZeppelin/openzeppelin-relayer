...
For detailed signer configuration options, see the [Signers](/relayer/1.7.x/configuration/signers) guide.
...
For a step-by-step setup, see [Quick Start Guide](/relayer/1.7.x/quickstart).
Key prerequisites:
...
- `include_revert_data`: Enable/disable best-effort recovery of the on-chain revert payload into a failed transaction's `status_reason`. Disabled by default; set to `true` to opt in. Recovery issues extra RPC calls on every failed transaction, preferring `debug_traceTransaction` and falling back to `eth_call` at the execution block. Before enabling, confirm your provider supports these methods — many hosted/public RPCs do not expose `debug_traceTransaction`
...
1. **_Primary Method_**: Uses the RPC `estimate_gas` method to calculate gas requirements
   - The estimated value is increased by 10% as a safety buffer
   - Provides accurate estimates for most transaction types
2. **_Fallback Method_**: When RPC estimation fails, default gas limits are applied based on transaction type:
   - **_Simple ETH transfer_** (no data): 21,000 gas
   - **_ERC20 transfer_** (`0xa9059cbb`): 65,000 gas
   - **_ERC721/ERC20 transferFrom_** (`0x23b872dd`): 80,000 gas
   - **_Complex contracts_** (all other function calls): 200,000 gas
...
- Configure `RPC_ALLOWED_HOSTS` and `RPC_BLOCK_PRIVATE_IPS` for SSRF protection. See [Configuration → RPC URL Security](/relayer/configuration#rpc-url-security)
