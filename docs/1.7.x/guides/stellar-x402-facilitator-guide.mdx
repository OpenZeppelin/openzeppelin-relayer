...
- **Assets**: Configure the contract addresses of assets you want to accept payments in. The address `CBIELTK6YBZJU5UP2WWQEUCYKLPU6AUNZ2BQ4WWFEIE3USCIHMXQDAMA` used throughout this guide is the [USDC token contract on Stellar testnet](https://stellar.expert/explorer/testnet/contract/CBIELTK6YBZJU5UP2WWQEUCYKLPU6AUNZ2BQ4WWFEIE3USCIHMXQDAMA). For mainnet, use the corresponding mainnet USDC contract address
...
### x402 with Stellar Channels Service

For high-throughput x402 payment settlement, you can connect the x402 Facilitator plugin with the [OpenZeppelin Stellar Channels Service](/relayer/1.7.x/guides/stellar-channels-guide). The Channels service provides managed infrastructure for parallel transaction submission on Stellar, handling fee management and sequence number coordination automatically.

**Benefits of using Channels with x402:**
...
To connect the x402 Facilitator plugin with the Channels service, add the `channel_service_api_url` and `channel_service_api_key` fields to your relayer configuration. Below is a full configuration example:

```json
{
  "relayers": [
    {
      "id": "x402-test-relayer",
      "name": "X402 Test Relayer",
      "network": "testnet",
      "paused": false,
      "network_type": "stellar",
      "signer_id": "local-signer",
...
- **`channel_service_api_key`**: Your API key for the Channels service. See [Get Your API Key](/relayer/1.7.x/guides/stellar-channels-guide#1-get-your-api-key) for instructions on generating one.
...
For more details on the Channels service, see the [Stellar Channels Guide](/relayer/1.7.x/guides/stellar-channels-guide) and the [Channels Plugin documentation](/relayer/1.7.x/plugins/channels).

### Adding Trustlines for Token Support

<Callout>
  This is only needed if the relayer address needs to hold tokens like USDC. If
  the relayer is only used for transaction submission and does not receive or
  hold the asset, no trustline is required.
</Callout>

Before a Stellar account can hold or receive tokens like USDC, it must establish a **trustline** to the token's contract. You can create a trustline using [Stellar Laboratory](https://laboratory.stellar.org/) to build a `Change Trust` transaction, then submit the XDR via the relayer:
