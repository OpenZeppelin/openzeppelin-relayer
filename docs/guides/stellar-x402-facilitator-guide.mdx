...
When `raw_response: true` is set (recommended), the plugin returns responses directly without the standard Relayer `ApiResponse` wrapper. This matches what most x402 client libraries expect.
