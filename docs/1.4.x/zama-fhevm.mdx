...
Because the OpenZeppelin Relayer treats these as regular EVM networks, any network configuration that works for EVM will work here. See the [Network Configuration](/relayer/1.4.x/configuration/network) guide for the full list of options.
...
For signer configuration details, see the [Signers](/relayer/1.4.x/configuration/signers) guide.
...
For a full working walkthrough, see the [Zama FHEVM Counter Guide](/relayer/1.4.x/guides/zama-fhevm-counter-guide).
...
| Method | Purpose |
| --- | --- |
| [Send Transaction](/relayer/1.4.x/api/sendTransaction) | Submit an encrypted FHEVM contract call on-chain |
| [Sign Typed Data](/relayer/1.4.x/api/signTypedData) | Sign the EIP-712 payload used for user decryption |
| [Get Relayer](/relayer/1.4.x/api/getRelayer) | Retrieve the OpenZeppelin Relayer's EVM address (needed by the Zama SDK for input proofs and decryption requests) |
| [Get Transaction by ID](/relayer/1.4.x/api/getTransactionById) | Poll for transaction status after submission |
...
- [Zama FHEVM Counter Guide](/relayer/1.4.x/guides/zama-fhevm-counter-guide)
