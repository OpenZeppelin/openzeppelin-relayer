...
The file contains a list of plugins. Each plugin supports the following configuration fields:

- `id` (string, required): Unique identifier for the plugin.
- `path` (string, required): Plugin entrypoint path relative to the relayer `plugins/` directory.
- `timeout` (number, optional): Maximum execution time in seconds. Defaults to 300.
- `emit_logs` (boolean, optional): When `true`, include plugin logs in `metadata.logs` in the HTTP response.
- `emit_traces` (boolean, optional): When `true`, include relayer API traces in `metadata.traces` in the HTTP response.
- `raw_response` (boolean, optional): When `true`, return the plugin result directly (no `ApiResponse` envelope).
- `allow_get_invocation` (boolean, optional): When `true`, allow `GET` requests to invoke the plugin.
- `config` (object, optional): User-defined configuration (a JSON object) exposed to the plugin as `context.config`.
