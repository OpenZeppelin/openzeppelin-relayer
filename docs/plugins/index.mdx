...
```bash
# 2. HTTP request timeout (must be >= plugin timeout)
export REQUEST_TIMEOUT_SECONDS=120
```

<Callout type="warn">
Ensure `REQUEST_TIMEOUT_SECONDS` is **at least as large** as your longest plugin `timeout`. If it's shorter, Actix will close the HTTP connection while the plugin is still running, causing `write EPIPE` errors.
</Callout>
...
|-------|-------|----------|
| `write EPIPE` / `Uncaught exception: Error: write EPIPE` | HTTP timeout while plugin still running | Set `REQUEST_TIMEOUT_SECONDS` >= your plugin `timeout` in config.json |
...
| `ScriptTimeout(N)` | Plugin execution exceeded timeout | Increase `timeout` in plugin config (config.json) and ensure `REQUEST_TIMEOUT_SECONDS` >= that value |
