...
## 15. Shielded Observation — PR-3 v3 (Resolved)

The `Transient` decision in §14 closed off **shield/unshield** ops, but
left **shielded observation** (ingesting incoming shielded transfers
into the wallet's spendable set) blocked under a different constraint.
PR-3 v3 resolves the observation problem and unblocks the live shielded
relay path. This section captures the diagnosis and fix.

### 15.1 The problem

Earlier sync code routed every shielded tx delivered by the indexer's
`shieldedTransactions` subscription through
`LedgerContext::update_from_tx`, which strict-verifies **every proof in
the tx** — DUST-spend, signatures, native, contract — against the
LedgerContext's own DUST/tblock state.

For txs we **observe** (received from someone else, already accepted by
the chain), strict re-verification fails on `InvalidDustSpendProof`:
the sender generated their DUST proof against their own DUST tree
state, and our LedgerContext can't reproduce the same reference state.
Result: the tx is rejected on apply, the shielded coin never enters our
wallet's zswap state, the relayer can't see or spend it.

This is distinct from the building-side concern §2.9 addresses. There,
the `SharedDustSyncTask` keeps DUST state live so **outgoing** proofs
we generate land against the chain's current DUST state. That sharing
optimisation does not help here — observed-tx proofs were generated by
the sender against their state, not ours.

### 15.2 The reference SDK behaviour

A Node spike (`scripts/midnight-shielded-spike/`) reproduced the same
sync flow using the official `@midnight-ntwrk/wallet-sdk-shielded`
package against the same indexer, viewing key, and 32-byte seed. It
ingested the shielded tx cleanly with no proof rejection. Comparing
the two paths:

| Aspect | Our Rust path (broken) | TS reference (works) |
|---|---|---|
| Subscription | `shieldedTransactions(sessionId, index)` — per-wallet, full tx bytes | `zswapLedgerEvents(id: 0)` — global per-event stream |
| Apply fn | helpers' `update_from_tx` (strict verify) | `CoreWallet.replayEventsWithChanges` (decrypt + apply) |
| Proof verification | All proofs re-checked | Skipped — chain already accepted |

Both produce equivalent state after observation; the TS path is
strictly looser about what it requires from local state at apply time.

### 15.3 The fix

helpers' `Wallet::update_state_from_offers` is the Rust equivalent of
TS's `state.apply(secretKeys, &offer)` loop:
...
This is **already called inside `update_from_tx`** (after successful
verification, line 380 of `helpers/src/versions/common/context.rs`),
but only after the strict-verify gate that fails for observed txs.

PR-3 v3 adds `LedgerContextManager::apply_observed_tx_offers` that:

1. Deserializes the tx bytes (`MnFinalizedTransaction`).
2. Matches `Transaction::Standard(stx)` (skips `ClaimRewards`, no
   shielded offers).
3. Collects every offer: `stx.guaranteed_coins` + each
   `stx.fallible_coins` segment.
4. Calls `LedgerContext::with_wallet_from_seed(seed, |w|
   w.update_state_from_offers(&offers))` — bypasses verification
   entirely; the chain has already validated the tx.

`sync_shielded`'s event handler is wired to call this method instead
of the strict path. The wallet's `shielded.state.coins` now grows with
each observed shielded transfer; `shielded_balances()` aggregates
those for the API.

### 15.4 What this enables

- **`/balance` and `/status`** surface incoming shielded balances under
  `TokenBalance { privacy: Shielded, token: <hex> }`. Verified
  end-to-end on preview testnet: a 10-tNIGHT shielded transfer from
  Lace lands in the relayer's `availableCoins` and shows up in the
  API.
- **Shielded-to-shielded relays** become possible (PR-3 v2's
  retargeting wrapper path can now be exercised — the wallet has
  shielded coins to spend).
- The **shield-op limitation from §14 still holds**: the relayer
  cannot create shielded coins from unshielded inputs. External
  shielding (Lace) remains the only path to seed shielded funds.

### 15.5 Concurrent-prepare race surfaces

With shielded ingestion working, the latent concurrent-prepare race
flagged earlier becomes load-bearing: two prepares for the same
relayer can both spend the same shielded coin if they read the wallet
state before either tx's nullifier reaches the indexer. Mitigations
to consider when shielded relay traffic ramps up:

1. Per-relayer prepare mutex (serializes builds for one relayer;
   parallelism preserved across relayers).
2. Optimistic nullifier marking (after a successful prove, register
   the consumed coins' nullifiers in a per-relayer pending set so
   subsequent coin selection skips them). This is the pattern the
   TS reference uses via `state.pending_spends`.

Neither is in PR-3 v3 — the immediate need was unblocking ingestion.

### 15.6 Decision log

- **Date:** 2026-04-28
- **Decision:** Bypass `update_from_tx` for observed shielded txs;
  use `Wallet::update_state_from_offers` directly. Surface shielded
  balances in `/balance` (and now `/status`).
- **Rationale:** Strict re-verification of foreign txs has no
  correctness benefit (chain already verified) and breaks ingestion
  in the common case where local DUST state diverges from the
  sender's. The narrower path mirrors the TS reference exactly.
- **Revisit when:** concurrent-prepare race surfaces under load
  (§15.5), or the indexer's `zswapLedgerEvents` global subscription
  becomes preferable to per-wallet `shieldedTransactions` for
  scaling reasons.

### 15.7 Note on §2.9

§2.9 framed `SharedDustSyncTask` as the cure for "DUST staleness
window that caused error 170 in earlier iterations." That framing
applies to **transaction building** — outgoing proofs we generate
need DUST state aligned with the chain's. It does **not** apply to
observation — incoming proofs were generated by someone else against
*their* state and are not reproducible against ours regardless of
shared-task freshness. The two paths have different staleness
sensitivities; PR-3 v3 separates them with `apply_observed_tx_offers`
