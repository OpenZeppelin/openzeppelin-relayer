...
- `include_revert_data`: Enable/disable best-effort recovery of the on-chain revert payload into a failed transaction's `status_reason`. Enabled by default; set to `false` to avoid the extra RPC calls issued when a transaction fails
