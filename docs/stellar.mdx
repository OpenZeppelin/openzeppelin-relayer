...
| --- | --- | --- |
| `strategies` | array | DEX strategies to use for token swaps, in priority order. Supported values: `"order-book"` (Stellar Horizon order book API), `"soroswap"` (Soroswap DEX - future implementation). Strategies are tried sequentially until one can handle the asset. |
...
**Note**: Sponsored transactions require the relayer to have a DEX service configured for token conversions. Ensure your relayer has access to a Stellar DEX service (e.g., Horizon order book API) for converting between XLM and fee tokens.
