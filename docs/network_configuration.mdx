...
| `status_check`            | object        | No       | Transaction status-check settings. `initial_delay_seconds` defaults to `8`; optional `retry_delay_seconds` sets the interval after a healthy, non-final check. Both accept `1`–`100` and inherit independently. Without a retry delay, the existing EVM backoff is preserved. |
...
```json
{
  ...
  "status_check": {
    ...
    "retry_delay_seconds": 2 // Faster follow-up checks on fast-finality chains
  },
...
<Callout type="warning">
  ...
  `R=2s` require at least 57 receives before adding that margin. Otherwise SQS
