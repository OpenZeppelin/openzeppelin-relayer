...
| `status_check.initial_delay_seconds` | number        | No       | Delay before the first status check, in seconds. Default `8`, range `1`–`100`.                                                           |
| `status_check.retry_delay_seconds`   | number        | No       | Delay between checks while a transaction is healthy but not final, in seconds. Default `8`, range `5`–`100`. Checks back off from this value to 1.5× it. |
...
Both `status_check` fields inherit independently from a parent network. A larger
`initial_delay_seconds` also delays the first resubmission and timeout evaluation,
because both run only during a status check.
...
<Callout type="warning">
  On SQS FIFO, every non-final check re-receives the same message, so a faster
  `retry_delay_seconds` raises `ApproximateReceiveCount` for long confirmations.
  Size the queue's `maxReceiveCount` for roughly `1 + confirmation_time / retry_delay`
  receives, or SQS can dead-letter the check and the transaction stops being monitored.
</Callout>
