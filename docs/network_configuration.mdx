...
- Configure **_any EVM-compatible network_** (Ethereum, Polygon, BSC, Arbitrum, Optimism, etc.)
- Set up **_Solana networks_** (mainnet-beta, devnet, testnet, custom RPC endpoints)
- Configure **_Stellar networks_** (Pubnet, Testnet, custom networks)
- Create **_custom network configurations_** with specific RPC endpoints, chain IDs, and network parameters
- Use **_inheritance_** to create network variants without duplicating configuration
...
Each JSON file **_must_** contain a top-level `networks` array:
...
| `optimism-based`          | Identifies Optimism-based networks using the OP Stack (e.g., Optimism, Base, World Chain)                                                                                                                             |
| `optimism` _(deprecated)_ | **_DEPRECATED_**: Use `optimism-based` instead. This tag will be removed in a future version.                                                                                                                         |
| `arbitrum-based`          | Identifies Arbitrum-based networks using the Arbitrum Stack                                                                                                                                                           |
| `no-mempool`              | Indicates networks that lack a traditional mempool (e.g., Arbitrum). Note: The relayer also treats networks tagged as `arbitrum-based` or `optimism-based` as lacking a mempool, even if `no-mempool` is not present. |
...
| `status_check`            | object        | No       | Transaction status-check settings. `initial_delay_seconds` defaults to `8` and accepts `1`–`100`; optional `retry_delay_seconds` accepts `5`–`100` and sets the interval after a healthy, non-final check. Both fields inherit independently. Without a retry delay, the existing EVM backoff is preserved. |
...
A larger `initial_delay_seconds` also delays the first resubmission and timeout
evaluation, because both run only during a status check.
...
```json
{
  ...
  "status_check": {
    "initial_delay_seconds": 8, // Delays the first status check
    "retry_delay_seconds": 5 // Faster follow-up checks on fast-finality chains
  },
...
<Callout type="warning">
  On SQS FIFO, each retry changes the visibility timeout of the same in-flight
  message, so every non-final check increments `ApproximateReceiveCount`. Size
  the queue's `maxReceiveCount` for the configured cadence and expected
  confirmation time: for confirmation time `T`, initial delay `I`, and retry
  delay `R`, budget at least `1 + ceil(max(0, T - I) / R)` receives, plus margin
  for outages and duplicate deliveries. For example, `T=120s`, `I=8s`, and
  `R=5s` require at least 24 receives before adding that margin. Otherwise SQS
  can dead-letter the status check and stop monitoring the transaction.
</Callout>

<Callout type="warning">
  Redis-backed deployments do not reseed network configuration after the
  bootstrap-completion marker exists. A changed `status_check` setting takes
  effect only after storage is cleared and reloaded, using
  `RESET_STORAGE_ON_START` or an explicit Redis wipe. The network update API
  cannot set these fields; it only updates `rpc_urls`.
</Callout>

The circuit-breaker budgets remain count-based: failed checks always use the
ordinary `STATUS_EVM_BACKOFF`, regardless of `retry_delay_seconds`, so the 25/75
failure budgets are deliberately not scaled with the healthy-check cadence. A
flaky RPC that alternates failures with successful non-final checks can compress
the 75-failure wall-clock window by up to about 2× because only the successful
checks become faster; a time-based breaker is a separate follow-up.
