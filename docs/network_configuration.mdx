...
| `status_check.retry_delay_seconds`   | number        | No       | Interval between checks while a transaction is healthy but not final, in seconds. Range `5`–`100`. Unset keeps the existing EVM backoff. |
