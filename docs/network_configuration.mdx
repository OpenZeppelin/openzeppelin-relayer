...
| `status_check_initial_delay_seconds` | number | No | Delay before the first transaction status check, in whole seconds. Defaults to `8`; valid range is `1`–`100`. Inherited when omitted. |
...
```json
{
  ...
  "status_check_initial_delay_seconds": 8, // Only affects the first status check
