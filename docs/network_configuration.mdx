...
| `status_check`            | object        | No       | Transaction status-check settings. `initial_delay_seconds` defaults to `8`, accepts `1`–`100`, and is inherited when omitted. Larger values also postpone the first resubmission and timeout evaluation. |
...
```json
{
  ...
  "status_check": { "initial_delay_seconds": 8 }, // Also delays the first resubmission and timeout evaluation
