...
|private_rpc_url
|String
|Optional private RPC URL for the network. If provided, this will be used instead of the public RPC URLs. This is useful for using your own RPC node or a paid service provider.
  ...
  "relayers": [
    {
      ...
      "signer_id": "local-signer",
      "network_type": "evm",
      "private_rpc_url": "https://your-private-rpc-url.example.com",
      "policies": {
    ...
    },
    {
      ...
      "signer_id": "local-signer",
      "network_type": "solana",
      "private_rpc_url": "https://your-private-solana-rpc.example.com",
      "policies": {
        "min_balance": 0,
        "allowed_tokens": [
          {
...
2. **Private RPC URL**: You can specify a custom RPC URL using the `private_rpc_url` field in the relayer configuration. This is useful when you want to:
   ...
   * Access private or custom network endpoints

When both are available, the relayer will:
1. First attempt to use the `private_rpc_url` if configured
2. Fall back to the public RPC URLs if no private URL is configured or if it's not accessible

[IMPORTANT]
====
When using private RPC URLs:
