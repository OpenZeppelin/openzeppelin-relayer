...
|enum(user | relayer)
|Defines which entity pays for transaction fees. When set to "user", the transaction sender pays fees. When set to "relayer", the relayer service covers transaction fees. Default is "user".

|solana
|fee_margin_percentage
|f32 (Values between 0 and 100)
