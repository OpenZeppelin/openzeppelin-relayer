...
|custom_rpc_url
|String
|Optional custom RPC URL for the network. If provided, this will be used instead of the public RPC URLs. This is useful for using your own RPC node or a paid service provider.
  ...
  "relayers": [
    {
      ...
      "signer_id": "local-signer",
      "network_type": "evm",
      "custom_rpc_url": "https://your-private-rpc-url.example.com",
      "policies": {
    ...
    },
    {
      ...
      "signer_id": "local-signer",
      "network_type": "solana",
      "custom_rpc_url": "https://your-private-solana-rpc.example.com",
      "policies": {
        "min_balance": 0,
        "allowed_tokens": [
          {
...
2. **Private RPC URL**: You can specify a custom RPC URL using the `custom_rpc_url` field in the relayer configuration. This is useful when you want to:
...
1. First attempt to use the `custom_rpc_url` if configured
