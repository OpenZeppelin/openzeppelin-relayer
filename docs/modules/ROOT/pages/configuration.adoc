...
| `BACKGROUND_WORKER_RATE_LIMIT`
| `3000`
| `<any positive number>`
| Global rate limit for all background workers (number of jobs processed per rate limit duration).

| `BACKGROUND_WORKER_RATE_LIMIT_DURATION_MS`
| `1000`
| `<any positive number>`
| Duration in milliseconds for the background worker rate limit window. Together with `BACKGROUND_WORKER_RATE_LIMIT`, controls how many jobs workers can process per time window.
...
| `1500`
| `<any positive number>`
| Maximum number of concurrent EVM transaction status check jobs that can be processed simultaneously. EVM status checker uses slower retries (8-20s) to avoid triggering premature resubmission.
...
| `1500`
| `<any positive number>`
| Maximum number of concurrent Solana token swap request jobs that can be processed simultaneously.
...
| `1500`
| `<any positive number>`
| Maximum number of concurrent relayer health check jobs that can be processed simultaneously.
