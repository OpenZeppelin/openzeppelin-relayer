...
|`midnight`
|Midnight blockchain networks. Supports Midnight testnet with indexer and prover integration.
|===
...
|string
|Yes
|Network type: `"evm"`, `"solana"`, `"stellar"` or `"midnight"`
...
|object
|Yes*
|Indexer service URLs with `ws` and `http` endpoints (*Required for base networks, optional for inherited)

|`prover_url`
|string
|Yes*
|URL for the Midnight prover service (*Required for base networks, optional for inherited)
|===
