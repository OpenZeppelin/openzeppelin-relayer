...
| `PUBSUB_TOPIC_PREFIX`                                              | `relayer-`               | `<topic prefix>`                                                            | Prefix for Pub/Sub topic and subscription names (topics `<prefix><queue>`, subscriptions `<prefix><queue>-sub`). Only applies when `QUEUE_BACKEND=pubsub`.                                                                                                                           |
...
When `QUEUE_BACKEND=pubsub`, the relayer **never creates Pub/Sub resources** — it probes them at startup and fails fast if any are missing. Pre-create one topic and one subscription per queue type (8 pairs) using the `relayer-` prefix (or your custom `PUBSUB_TOPIC_PREFIX`).
