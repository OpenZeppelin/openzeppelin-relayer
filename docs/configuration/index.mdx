...
| `QUEUE_BACKEND`                                                    | `redis`                  | `redis, sqs, pubsub`                                                        | Queue backend implementation. `redis` uses Apalis + Redis queues. `sqs` uses AWS SQS workers. `pubsub` uses GCP Pub/Sub workers (and still requires Redis for deferred-job scheduling).                                                                                              |
