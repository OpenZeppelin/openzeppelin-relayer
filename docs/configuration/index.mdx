...
| `SQS_QUEUE_TYPE`                                                   | `standard`               | `standard, fifo`                                                            | SQS queue type. `standard` offers higher throughput and native per-message delays. `fifo` provides message ordering and exactly-once delivery. Must match the actual queue type in AWS.                                                                                              |
...
When `QUEUE_BACKEND=sqs`, create queues with the `relayer-` prefix (or your custom `SQS_QUEUE_URL_PREFIX`). The queue type (standard or FIFO) is controlled by `SQS_QUEUE_TYPE` and must match the actual queues in AWS — startup validation will fail on a mismatch.

#### Standard queues (default, `SQS_QUEUE_TYPE=standard`)
...
#### FIFO queues (`SQS_QUEUE_TYPE=fifo`)
