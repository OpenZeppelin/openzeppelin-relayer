...
| `IDEMPOTENCY_KEY_TTL_SECONDS`                                      | `86400`                  | `number`                                                                    | Number of seconds an `Idempotency-Key` sent with `POST /relayers/{relayer_id}/transactions` stays reserved. A repeat request with the same key inside this window returns the original transaction instead of creating a new one. See [Idempotent Requests](./api#idempotent-requests).|
