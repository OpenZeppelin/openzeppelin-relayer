...
**_File-based Configuration:_**

1. **_`config.json`_**: Contains relayer definitions, signer configurations, and network policies
2. **_`.env`_** file: Contains environment variables like API keys and connection strings

**_API-based Configuration:_**
...
- See the **_API Reference_** page for detailed endpoints documentation
...
| `REDIS_URL`                                                        | `redis://localhost:6379` | `<redis connection string>`                                                 | Redis connection URL for the primary instance. Used for all write operations and read operations when `REDIS_READER_URL` is not set. See [Storage Configuration](./configuration/storage) for Redis setup details.                                                                   |
| `REDIS_READER_URL`                                                 | (none)                   | `<redis connection string>`                                                 | Optional separate endpoint for read operations. When set, read operations use this endpoint while writes use `REDIS_URL`. Useful for AWS ElastiCache with read replicas. See [Storage Configuration](./configuration/storage) for configuration examples.                            |
...
| `REDIS_POOL_MAX_SIZE`                                              | `500`                    | `<number>`                                                                  | Maximum number of connections in the primary Redis connection pool. See [Storage Configuration](./configuration/storage) for tuning guidance.                                                                                                                                        |
| `REDIS_READER_POOL_MAX_SIZE`                                       | `1000`                   | `<number>`                                                                  | Maximum number of connections in the reader Redis connection pool. Only used when `REDIS_READER_URL` is set. Useful for read-heavy workloads. See [Storage Configuration](./configuration/storage) for pool sizing recommendations.                                                  |
...
| `QUEUE_BACKEND`                                                    | `redis`                  | `redis, sqs, pubsub, rabbitmq`                                              | Queue backend implementation. `redis` uses Apalis + Redis queues. `sqs` uses AWS SQS workers. `pubsub` uses GCP Pub/Sub workers. `rabbitmq` uses RabbitMQ workers. The `sqs`/`pubsub`/`rabbitmq` backends still require Redis for deferred-job scheduling.                |
...
| `PUBSUB_PROJECT_ID`                                                | (none)                   | `<gcp project id>`                                                          | Required when `QUEUE_BACKEND=pubsub`. GCP project that holds the Pub/Sub topics and subscriptions; resolved for ADC and the Cloud Monitoring backlog read.                                                                                                                           |
| `PUBSUB_TOPIC_PREFIX`                                              | `relayer`                | `<topic prefix>`                                                            | Prefix for Pub/Sub topic and subscription names; the `-` separator is added automatically (topics `<prefix>-<queue>`, subscriptions `<prefix>-<queue>-sub`). Only applies when `QUEUE_BACKEND=pubsub`.                                                                               |
| `PUBSUB_EMULATOR_HOST`                                             | (none)                   | `<host:port>`                                                               | Optional. When set (example: `localhost:8085`), targets the local Pub/Sub emulator instead of real GCP and skips ADC resolution. Leave unset for real GCP.                                                                                                                           |
| `RABBITMQ_URL`                                                    | (none)                   | `<amqp uri>`                                                                | Required when `QUEUE_BACKEND=rabbitmq`. Full AMQP 0-9-1 URI (`amqp://user:pass@host:5672/vhost`, or `amqps://…` for TLS); standard query params like `?heartbeat=20` pass through. Embeds credentials, so it is redacted in all logs/errors (scheme+host+port+vhost only).            |
| `RABBITMQ_QUEUE_PREFIX`                                           | `relayer`                | `<queue prefix>`                                                            | Prefix for RabbitMQ queue names; the `-` separator is added automatically (queues `<prefix>-<queue>`). Only applies when `QUEUE_BACKEND=rabbitmq`.                                                                                                                                   |
| `RABBITMQ_PASSIVE_QUEUES`                                         | `false`                  | `bool` (`true/false`)                                                       | When `false` the relayer declares its 8 durable queues at startup (needs `configure`+`write`+`read`). When `true` it only verifies they exist (needs `write`+`read`) and fails fast naming anything missing — for locked-down brokers or pre-provisioned quorum queues.              |
...
| `RPC_BLOCK_PRIVATE_IPS`                                            | `false`                  | `bool`                                                                      | Block private IP addresses (RFC 1918, loopback, link-local) in RPC URLs. Set to `true` to prevent RPC URLs from targeting private networks.                                                                                                                                          |
| `CONNECTION_BACKLOG`                                               | `511`                    | `<number>`                                                                  | TCP listen backlog size (pending connections queue). Higher values allow more connections to be queued during traffic bursts, preventing connection drops. Default of 511 is a production-ready value that balances memory usage with burst capacity.                              |
| `REQUEST_TIMEOUT_SECONDS`                                           | `30`                     | `<seconds>`                                                                  | Request handler timeout in seconds for API endpoints. This is a security measure to prevent resource exhaustion attacks (DoS). It limits how long a request handler can run, preventing slowloris-style attacks and ensuring resources are freed promptly.                         |
| `RELAYER_CONCURRENCY_LIMIT`                                        | `100`                    | `<any positive number>`                                                     | Maximum number of concurrent requests allowed for `/api/v1/relayers/*` endpoints. When this limit is reached, additional requests will be rejected with a 429 Too Many Requests error. This helps prevent resource exhaustion and ensures fair resource allocation.                    |
| `MAX_CONNECTIONS`                                                  | `256`                    | `<any positive number>`                                                     | Maximum number of concurrent TCP connections allowed server-wide. This is a safety net that prevents the server from accepting more connections than it can handle. When this limit is reached, new connections will be rejected.                                                |
...
| `EVM_STATUS_CHECK_INITIAL_DELAY_SECONDS`                           | `8`                      | `0-60`                                                                      | Delay in seconds before the first status check of a new EVM transaction. Invalid values fall back to the default.                                                                                                                                                                    |
| `EVM_STATUS_RETRY_INITIAL_MS`                                      | `8000`                   | `500-60000`                                                                 | Initial delay in milliseconds between EVM transaction status checks. Invalid values fall back to the default.                                                                                                                                                                        |
| `EVM_STATUS_RETRY_MAX_MS`                                          | `12000`                  | `500-60000`                                                                 | Maximum delay in milliseconds between EVM transaction status checks. Clamped up to `EVM_STATUS_RETRY_INITIAL_MS` if set lower. Invalid values fall back to the default.                                                                                                              |
| `STELLAR_STATUS_CHECK_INITIAL_DELAY_SECONDS`                       | `2`                      | `0-60`                                                                      | Delay in seconds before the first status check of a new Stellar transaction. Invalid values fall back to the default.                                                                                                                                                                |
| `STELLAR_STATUS_RETRY_INITIAL_MS`                                  | `2000`                   | `500-60000`                                                                 | Initial delay in milliseconds between Stellar transaction status checks. Invalid values fall back to the default.                                                                                                                                                                    |
| `STELLAR_STATUS_RETRY_MAX_MS`                                      | `3000`                   | `500-60000`                                                                 | Maximum delay in milliseconds between Stellar transaction status checks. Clamped up to `STELLAR_STATUS_RETRY_INITIAL_MS` if set lower. Invalid values fall back to the default.                                                                                                      |
...
- `pubsub`: GCP Pub/Sub queue backend (requires Redis for deferred-job scheduling)
- `rabbitmq`: RabbitMQ queue backend (requires Redis for deferred-job scheduling)

The `redis` backend is the simplest to run and is a good default for single-instance and development setups. For high-throughput or multi-instance production deployments, we recommend `sqs`, `pubsub`, or `rabbitmq`.
...
# Optional (default: relayer; the "-" separator is added automatically)
# PUBSUB_TOPIC_PREFIX=relayer
# Real GCP: provide Application Default Credentials (service-account key shown)
GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account-key.json
# Or target the local emulator instead of real GCP (no ADC needed):
# PUBSUB_EMULATOR_HOST=localhost:8085
```

Pub/Sub has no native delayed delivery, so the backend still requires Redis (`REDIS_URL`): deferred jobs and retry backoff are held in a Redis sorted set and published to the topic when due.

Example for RabbitMQ:

```bash
QUEUE_BACKEND=rabbitmq
# Full AMQP URI (use amqps:// for TLS). Embeds credentials → redacted in logs.
RABBITMQ_URL=amqp://user:pass@host:5672/%2f
# Optional (default: relayer; the "-" separator is added automatically)
# RABBITMQ_QUEUE_PREFIX=relayer
# Optional: verify-only mode (never create queues) for locked-down brokers
# or pre-provisioned quorum queues. Default: false
# RABBITMQ_PASSIVE_QUEUES=false
```

Like Pub/Sub, RabbitMQ has no native arbitrary delayed delivery (and the
delayed-message-exchange plugin is archived / incompatible with RabbitMQ 4.3+),
so the backend still requires Redis (`REDIS_URL`): deferred jobs and retry
backoff are held in a Redis sorted set and published to the broker when due. See
the [RabbitMQ queue provisioning guide](#rabbitmq-queue-provisioning-guide).
...
| Queue | Visibility timeout (seconds) | Suggested maxReceiveCount (DLQ redrive) |
| --- | --- | --- |
| `transaction-request` | `30` | `6` |
| `transaction-submission` | `30` | `2` |
| `status-check` | `30` | `1000` (or a high value) |
| `status-check-evm` | `30` | `1000` (or a high value) |
| `status-check-stellar` | `20` | `1000` (or a high value) |
| `notification` | `60` | `6` |
| `token-swap-request` | `60` | `3` |
| `relayer-health-check` | `60` | `3` |
...
### Pub/Sub queue provisioning guide

When `QUEUE_BACKEND=pubsub`, the relayer **never creates Pub/Sub resources** — it probes them at startup and fails fast if any are missing. Pre-create one topic and one subscription per queue type (8 pairs) named `{prefix}-{queue}` and `{prefix}-{queue}-sub` (the default prefix is `relayer`, giving `relayer-transaction-request`, etc.; override with `PUBSUB_TOPIC_PREFIX`).

Required topic / subscription pairs:
...
No dead-letter topics are required: a bounded job that exhausts `max_retries` stops retrying, and the transaction's terminal state in the repository is the durable record (matching the Redis and SQS backends).

Create the resources with your own infrastructure (for example Terraform), or use the idempotent `scripts/provision-gcp.sh` helper from the [`gcp-pubsub-queue-storage` example](https://github.com/OpenZeppelin/openzeppelin-relayer/tree/main/examples/gcp-pubsub-queue-storage):

```bash
PUBSUB_PROJECT_ID=my-gcp-project ./scripts/provision-gcp.sh
```

#### Credentials and IAM

For real GCP, the relayer resolves Application Default Credentials (ADC) — a service-account key via `GOOGLE_APPLICATION_CREDENTIALS`, Workload Identity, or the GCE metadata server. The principal needs:

- `roles/pubsub.publisher` + `roles/pubsub.subscriber` (or `roles/pubsub.editor`)
- `roles/monitoring.viewer` — for the on-demand backlog-depth read

Set `PUBSUB_EMULATOR_HOST` (for example `localhost:8085`) to target the local [Pub/Sub emulator](https://cloud.google.com/pubsub/docs/emulator) instead; ADC is not required in that mode.

#### Scheduling, retries, and backlog depth

Pub/Sub has no native delayed delivery, so deferred jobs and retry backoff are held in a **Redis sorted set and published when due** — the Pub/Sub backend therefore requires `REDIS_URL` in addition to the Pub/Sub settings. Status-check queues are unbounded and re-run with backoff until the transaction reaches a final state.

Backlog depth is read from Cloud Monitoring (`subscription/num_undelivered_messages`) and exposed via the `queue_depth` gauge; under the emulator (which does not serve Cloud Monitoring) it is reported as unavailable rather than `0`.

### RabbitMQ queue provisioning guide

When `QUEUE_BACKEND=rabbitmq`, the relayer maps each of the 8 queue types to **one durable classic queue** named `{prefix}-{queue}` (default prefix `relayer`, override with `RABBITMQ_QUEUE_PREFIX`), published via the **default exchange** (`routing_key = queue name`). There are no custom exchanges or bindings.
...
Queue properties (what the relayer declares, and what pre-provisioned queues must match in passive mode): `durable=true`, `exclusive=false`, `auto_delete=false`, no extra arguments. No dead-letter exchanges are required — a bounded job that exhausts `max_retries` stops retrying and the transaction's terminal state in the repository is the durable record (matching the other backends).

#### Resource policy: declare vs verify-only

- **Default (`RABBITMQ_PASSIVE_QUEUES=false`):** the relayer **declares** all 8 queues idempotently at startup. The broker user needs `configure` + `write` + `read` on `^{prefix}-.*`.
- **Verify-only (`RABBITMQ_PASSIVE_QUEUES=true`):** the relayer issues **passive** declares — it checks each queue exists and fails fast naming any missing one, never creating anything. Needs only `write` + `read`. Use this for locked-down brokers, or to pre-provision **quorum** queues (a non-passive declare against a quorum queue fails with `406 inequivalent arg` by design — quorum queues are always durable, so the relayer's persistent publishing is correct on them).

#### TLS and credentials

Use `amqps://…` for TLS; tune the heartbeat with the standard URI query parameter (e.g. `?heartbeat=20`). `RABBITMQ_URL` embeds the broker password, so the relayer redacts it in every log and error message (logging scheme + host + port + vhost only). The default `guest` user is restricted to loopback connections by the broker; create a dedicated user with scoped permissions for non-local deployments.

#### Scheduling, retries, and backlog depth

RabbitMQ has no native arbitrary delayed delivery (the delayed-message-exchange plugin is archived and incompatible with RabbitMQ 4.3+), so deferred jobs and retry backoff are held in a **Redis sorted set and published when due** — the RabbitMQ backend therefore requires `REDIS_URL`. Publishes are publisher-confirmed (produce returns only after the broker acks a persistent message to a durable queue). Backlog depth is read with a passive `queue_declare` over the live connection and exposed via the `queue_depth` gauge (no management API required); while disconnected it is reported as unavailable rather than `0`. Works on self-hosted RabbitMQ 3.13+/4.x, Amazon MQ for RabbitMQ, and CloudAMQP — **no broker plugins required**. See the [`rabbitmq-queue-storage` example](https://github.com/OpenZeppelin/openzeppelin-relayer/tree/main/examples/rabbitmq-queue-storage) for a one-command local broker setup.
...
All components defined in `config.json` can also be managed via REST API endpoints. This provides runtime flexibility for adding, updating, or removing relayers, signers, and notifications without restarting the service. See the **_API Reference_** page for detailed endpoints documentation.
...
| evm | include_revert_data | `bool` | Best-effort recovery of the on-chain revert payload into a failed transaction's `status_reason`. Disabled by default; set to `true` to opt in. Issues extra RPC calls on every failed transaction (preferring `debug_traceTransaction`, falling back to `eth_call`); confirm your provider supports these methods before enabling, as many hosted/public RPCs do not expose `debug_traceTransaction`. (Optional) |
...
1. **_Public RPC URLs_**: These are the default RPC endpoints provided by the network. They are automatically selected based on the network configuration.
2. **_Custom RPC URLs_**: You can specify custom RPC URLs using the `custom_rpc_urls` field in the relayer configuration. Each URL can be configured with an optional weight for high availability:
