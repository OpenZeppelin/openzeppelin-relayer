...
| evm | include_revert_data | `bool` | Best-effort recovery of the on-chain revert payload into a failed transaction's `status_reason`. Enabled by default; set to `false` to skip the extra RPC calls issued when a transaction fails. (Optional) |
