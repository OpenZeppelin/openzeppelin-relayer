...
| `QUEUE_BACKEND`                                                    | `redis`                  | `redis, sqs`                                                                | Queue backend implementation. `redis` uses Apalis + Redis queues. `sqs` uses AWS SQS workers and queue backend abstraction.                                                                                                                                                          |
...
- `sqs`: AWS SQS queue backend (FIFO queues expected)
...
When `QUEUE_BACKEND=sqs`, create **FIFO** queues with the `relayer-` prefix (or your custom `SQS_QUEUE_URL_PREFIX`).
...
| Queue | Queue type | Visibility timeout (seconds) | Suggested maxReceiveCount (DLQ redrive) |
| --- | --- | --- | --- |
| `transaction-request` | FIFO | `30` | `6` |
| `transaction-submission` | FIFO | `30` | `2` |
| `status-check` | FIFO | `30` | `1000` (or a high value) |
| `status-check-evm` | FIFO | `30` | `1000` (or a high value) |
| `status-check-stellar` | FIFO | `20` | `1000` (or a high value) |
| `notification` | FIFO | `60` | `6` |
| `token-swap-request` | FIFO | `60` | `3` |
| `relayer-health-check` | FIFO | `60` | `3` |

Notes:

- `FifoQueue` must be `true`. Startup validation fails if a queue is not FIFO.
- The relayer sets `MessageGroupId` and `MessageDeduplicationId` on send.
- Status-check queues use short-delay re-enqueue for retries; use a high `maxReceiveCount` to avoid premature DLQ movement.
- Configure a DLQ per queue in production (FIFO queue -> FIFO DLQ).

#### High-throughput FIFO mode

For production workloads, enable **high-throughput FIFO** on the transaction-request, transaction-submission, and all status-check queues. This raises the per-queue throughput limit from 300 messages/second to 70,000 messages/second by partitioning throughput across message groups.

Set these additional attributes when creating the queue:

- `DeduplicationScope=messageGroup`
- `FifoThroughputLimit=perMessageGroupId`

The relayer already uses per-transaction `MessageGroupId` values for status checks and per-relayer groups for EVM transactions, so high-throughput mode benefits these queues directly. Low-volume queues (`notification`, `token-swap-request`, `relayer-health-check`) do not require this setting.
...
Then set redrive policy to attach a DLQ:

```bash
aws sqs set-queue-attributes \
  --queue-url "https://sqs.us-east-1.amazonaws.com/<ACCOUNT_ID>/relayer-transaction-request.fifo" \
