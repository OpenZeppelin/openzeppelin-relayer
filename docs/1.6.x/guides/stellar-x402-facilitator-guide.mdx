...
For high-throughput x402 payment settlement, you can connect the x402 Facilitator plugin with the [OpenZeppelin Stellar Channels Service](/relayer/1.6.x/guides/stellar-channels-guide). The Channels service provides managed infrastructure for parallel transaction submission on Stellar, handling fee management and sequence number coordination automatically.
...
- **`channel_service_api_key`**: Your API key for the Channels service. See [Get Your API Key](/relayer/1.6.x/guides/stellar-channels-guide#1-get-your-api-key) for instructions on generating one.
...
For more details on the Channels service, see the [Stellar Channels Guide](/relayer/1.6.x/guides/stellar-channels-guide) and the [Channels Plugin documentation](/relayer/1.6.x/plugins/channels).
