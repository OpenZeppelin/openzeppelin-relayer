...
For detailed signer configuration options, see the [Signers](/relayer/1.6.x/configuration/signers) guide.
...
For a step-by-step setup, see [Quick Start Guide](/relayer/1.6.x/quickstart).
Key prerequisites:
...
- Configure `RPC_ALLOWED_HOSTS` and `RPC_BLOCK_PRIVATE_IPS` for SSRF protection. See [Configuration → RPC URL Security](/relayer/1.6.x/configuration#rpc-url-security)
