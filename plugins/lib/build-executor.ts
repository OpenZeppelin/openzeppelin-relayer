#!/usr/bin/env ts-node
/**
 * Build script to pre-compile pool-executor.ts
 *
 * Run this during build/release to generate pool-executor.js
...
const INPUT_PATH = path.resolve(__dirname, 'pool-executor.ts');
const OUTPUT_PATH = path.resolve(__dirname, 'pool-executor.js');
const HASH_PATH = path.resolve(__dirname, 'pool-executor.js.sha256');
