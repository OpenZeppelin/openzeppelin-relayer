...
const hashPath = path.resolve(__dirname, 'sandbox-executor.js.sha256');
...
function writeHashFile(hash: string): void {
  const content = `${hash}  sandbox-executor.js\n`;
...
async function build(): Promise<void> {
  ...
  console.log('=== Building sandbox-executor ===');
  ...
  tempOutputPath = path.join(os.tmpdir(), `sandbox-executor-${crypto.randomUUID()}.js`);
