...
impl std::fmt::Display for RpcSelectorError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            RpcSelectorError::ClientInitializationError(e) => write!(f, "Client initialization error: {}", e),
            RpcSelectorError::WeightedIndexError(e) => write!(f, "Weighted index error: {}", e),
            RpcSelectorError::AllProvidersFailed => write!(f, "All available providers have failed"),
        }
    }
}
...
impl RpcSelector {
    ...
    fn create_weights_distribution(
        ...
        let weights: Vec<u32> = configs.iter().enumerate().map(|(idx, config)| {
        ...
        if let Some(first_weight) = first_non_zero_weight {
            let all_equal = weights.iter().enumerate()
    ...
    pub fn get_client<T>(
        ...
        initializer(url).map_err(|e| {
            RpcSelectorError::ClientInitializationError(format!("Client initialization failed: {}", e))
        })
    }
}
...
mod tests {
    ...
    fn test_mark_current_as_failed_single_provider() {
        ...
        assert!(matches!(next_url.unwrap_err(), RpcSelectorError::AllProvidersFailed));
    ...
    fn test_mark_current_as_failed_multiple_providers() {
        ...
        assert!(url4.is_err());
        assert!(matches!(url4.unwrap_err(), RpcSelectorError::AllProvidersFailed));
    }
    ...
    fn test_mark_current_as_failed_weighted() {
        ...
        let configs = vec![
            RpcConfig {
                url: "https://example1.com/rpc".to_string(),
                weight: Some(1), // Low weight
            },
            RpcConfig {
                url: "https://example2.com/rpc".to_string(),
                weight: Some(10), // High weight
            },
        ];
    ...
    fn test_auto_reset_mechanism() {
        ...
        let result = selector.select_url();
        assert!(result.is_ok(), "Providers should have been auto-reset after timeout");
    }
