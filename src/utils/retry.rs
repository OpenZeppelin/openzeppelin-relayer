    ...
    I: Fn(&str) -> Result<P, E>,
{
    ...
    while failover_count <= max_failovers && selector.available_provider_count() > 0 {
        ...
        let (provider, provider_url) = match get_provider(selector, operation_name, &provider_initializer) {
        ...
        log::debug!("Selected provider: {} for operation '{}'", provider_url, operation_name);
    ...
    I: Fn(&str) -> Result<P, E>,
{
    ...
    let provider_url = selector.get_client(|url| Ok::<_, eyre::Report>(url.to_string()))
        .map_err(|e| {
    ...
    let provider = provider_initializer(&provider_url)
        .map_err(|e| {
            log::warn!(
                "Failed to initialize provider '{}' for operation '{}': {}", 
                provider_url, operation_name, e
            );
            e
        })?;
...
mod tests {
    ...
    fn test_calculate_retry_delay() {
        ...
        for (i, expected) in expected_backoffs.iter().enumerate() {
            ...
            assert!(
                ...
                "Delay {} outside expected range {}..={}", delay.as_millis(), min_expected, max_expected
            );
        }
        ...
        assert!(delay.as_millis() <= (max_delay_ms as f64 * (1.0 + RETRY_JITTER_PERCENT)).ceil() as u128);
    }
    ...
    fn test_apply_jitter() {
        ...
        assert!(
            ...
            "Jittered value {} outside expected range {}..={}", jittered.as_millis(), min_expected, max_expected
        );
    ...
    fn test_get_provider() {
        ...
        let initializer = |url: &str| -> Result<String, TestError> {
            Ok(format!("provider-{}", url))
        };
    ...
    async fn test_retry_rpc_call_success() {
        ...
        let provider_initializer = |_url: &str| -> Result<String, TestError> {
    ...
    async fn test_retry_rpc_call_with_provider_failover() {
        ...
        assert!(final_provider.contains("8546"), "Wrong provider selected: {}", final_provider);
    }
    ...
    async fn test_retry_rpc_call_all_providers_fail() {
        ...
        let provider_initializer = |_: &str| -> Result<String, TestError> {
            Ok("mock_provider".to_string())
        };
