//! # EVM Google Could KMS Signer Implementation
//!
//! This module provides an EVM signer implementation that uses the Google Cloud KMS API
...
use alloy::{
    ...
    primitives::{eip191_hash_message, keccak256, PrimitiveSignature as Signature, B256},
};
...
use crate::{
    ...
    services::{
        GoogleCloudKmsService, GoogleCloudKmsServiceTrait, Signer, TurnkeyService,
        TurnkeyServiceTrait,
    },
};
...
fn decode_signature(raw: Vec<u8>) -> Result<ecdsa::Signature, SignerError> {
    let sig = ecdsa::Signature::from_der(raw.as_ref())
        .map_err(|e| SignerError::SigningError(format!("DER parse error: {e}")))?;
...
impl<T: GoogleCloudKmsServiceTrait> Signer for GoogleCloudKmsSigner<T> {
    ...
    async fn sign_transaction(
        ...
        // Sign the data with Google Cloud KMS service
        let signed_bytes = self
            .google_cloud_kms_service
            .sign_evm(&unsigned_tx_bytes)
            .await
            .map_err(|e| SignerError::SigningError(e.to_string()))?;

        let normalized_sig = decode_signature(signed_bytes.clone())
            .map_err(|e| SignerError::SigningError(format!("Failed to decode signature: {e}")))?;

        let r = normalized_sig.r().to_bytes();
        let s = normalized_sig.s().to_bytes();

        // Compute the message hash (what was signed)
        let message_hash = keccak256(&unsigned_tx_bytes);

        // Get the expected address (from KMS public key)
...
#[async_trait]
impl<T: GoogleCloudKmsServiceTrait> DataSignerTrait for GoogleCloudKmsSigner<T> {
